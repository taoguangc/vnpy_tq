"""STRAT_REV_OPP13_01@0.1.0 — PAAF orchestration for OPP13 day-boundary single-touch.

CID_010 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP13 Detector → levels → Execution → log.
Morphology lives in OPP13DayBoundaryTouchDetector.
...
from strategies.paaf.detectors.opp13_day_boundary_touch import (
    DEFAULT_CLOSE_RATIO,
    DEFAULT_DAY_BOUNDARY_TOLERANCE,
    DEFAULT_SHADOW_RATIO,
...
class StratRevOpp1301Strategy(CtaTemplate):
    """CID_010 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    day_boundary_tolerance = DEFAULT_DAY_BOUNDARY_TOLERANCE
    boundary_reversal_shadow_ratio = DEFAULT_SHADOW_RATIO
    boundary_reversal_close_ratio = DEFAULT_CLOSE_RATIO
    risk_reward = 1.0
    max_hold_bars = 50
    fixed_size = 1

    parameters = [
        "day_boundary_tolerance",
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP13DayBoundaryTouchDetector(
            day_boundary_tolerance=float(self.day_boundary_tolerance),
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_010; detector=OPP13@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
