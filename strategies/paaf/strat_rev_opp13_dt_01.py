"""STRAT_REV_OPP13_DT_01@0.1.0 — PAAF orchestration for OPP13 day-high double-top.

CID_012 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP13_DT Detector → levels → Execution → log.
Morphology lives in OPP13DayHighDoubleTopDetector.
...
from strategies.paaf.detectors.opp13_day_high_double_top import (
    DEFAULT_BOUNDARY_CLOSE_RATIO,
    DEFAULT_BOUNDARY_SHADOW_RATIO,
    DEFAULT_DAY_HIGH_LH_MAX_TICKS,
    DEFAULT_DAY_HIGH_SECOND_TEST_MAX_BARS,
    DEFAULT_DAY_HIGH_SECOND_TEST_TICKS,
    DEFAULT_QUALITY_CLOSE_FROM_HIGH_RATIO,
    DEFAULT_QUALITY_SHADOW_RATIO,
...
class StratRevOpp13Dt01Strategy(CtaTemplate):
    """CID_012 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    day_high_second_test_ticks = DEFAULT_DAY_HIGH_SECOND_TEST_TICKS
    day_high_second_test_max_bars = DEFAULT_DAY_HIGH_SECOND_TEST_MAX_BARS
    day_high_lh_max_ticks = DEFAULT_DAY_HIGH_LH_MAX_TICKS
    quality_shadow_ratio = DEFAULT_QUALITY_SHADOW_RATIO
    quality_close_from_high_ratio = DEFAULT_QUALITY_CLOSE_FROM_HIGH_RATIO
    boundary_reversal_shadow_ratio = DEFAULT_BOUNDARY_SHADOW_RATIO
    boundary_reversal_close_ratio = DEFAULT_BOUNDARY_CLOSE_RATIO
    risk_reward = 1.0
    max_hold_bars = 50
    fixed_size = 1

    parameters = [
        "day_high_second_test_ticks",
        "day_high_second_test_max_bars",
        "day_high_lh_max_ticks",
        "quality_shadow_ratio",
        "quality_close_from_high_ratio",
        "boundary_reversal_shadow_ratio",
        "boundary_reversal_close_ratio",
        "risk_reward",
        "max_hold_bars",
        "fixed_size",
    ]
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP13DayHighDoubleTopDetector(
            day_high_second_test_ticks=float(self.day_high_second_test_ticks),
            day_high_second_test_max_bars=int(self.day_high_second_test_max_bars),
            day_high_lh_max_ticks=float(self.day_high_lh_max_ticks),
            quality_shadow_ratio=float(self.quality_shadow_ratio),
            quality_close_from_high_ratio=float(self.quality_close_from_high_ratio),
            boundary_reversal_shadow_ratio=float(self.boundary_reversal_shadow_ratio),
            boundary_reversal_close_ratio=float(self.boundary_reversal_close_ratio),
        )
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_012; detector=OPP13_DT@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
