"""STRAT_TREND_OPP02_01@0.1.0 — PAAF orchestration for OPP02 EMA-pullback.

CID_008 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP02 Detector → levels → Execution → log.
Morphology lives in OPP02EmaPullbackDetector.
...
from strategies.paaf.detectors.opp02_ema_pullback import (
    DEFAULT_ATR_PERIOD,
    DEFAULT_EMA_PERIOD,
    DEFAULT_EMA_PULLBACK_MIN_BODY_RATIO,
    DEFAULT_EMA_PULLBACK_TOUCH_ATR,
    DEFAULT_WICK_MAX_FRACTION,
    OPP02EmaPullbackDetector,
)
...
class StratTrendOpp0201Strategy(CtaTemplate):
    """CID_008 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    ema_pullback_touch_atr = DEFAULT_EMA_PULLBACK_TOUCH_ATR
    ema_pullback_min_body_ratio = DEFAULT_EMA_PULLBACK_MIN_BODY_RATIO
    wick_max_fraction = DEFAULT_WICK_MAX_FRACTION
    risk_reward = 1.0
    max_hold_bars = 50
    fixed_size = 1
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP02EmaPullbackDetector(
            atr_period=int(self.atr_period),
            ema_period=int(self.ema_period),
            ema_pullback_touch_atr=float(self.ema_pullback_touch_atr),
            ema_pullback_min_body_ratio=float(self.ema_pullback_min_body_ratio),
            wick_max_fraction=float(self.wick_max_fraction),
        )
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_008; detector=OPP02@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
