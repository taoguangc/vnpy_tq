"""STRAT_REV_OPP17_01@0.1.0 — PAAF orchestration for OPP17 climax-reversal.

CID_005 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP17 Detector → levels → Execution → log.
Morphology lives in OPP17ClimaxReversalDetector.
...
from strategies.paaf.detectors.opp17_climax_reversal import (
    DEFAULT_ATR_PERIOD,
    DEFAULT_CLIMAX_RANGE_ATR,
    OPP17ClimaxReversalDetector,
)
...
class StratRevOpp1701Strategy(CtaTemplate):
    """CID_005 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    climax_range_atr = DEFAULT_CLIMAX_RANGE_ATR
    risk_reward = 1.0
    max_hold_bars = 50
    fixed_size = 1
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP17ClimaxReversalDetector(
            atr_period=int(self.atr_period),
            climax_range_atr=float(self.climax_range_atr),
        )
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_005; detector=OPP17@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
