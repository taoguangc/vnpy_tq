"""STRAT_REV_OPP12_01@0.1.0 — PAAF orchestration for OPP12 overshoot-fail.

CID_004 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP12 Detector → levels → Execution → log.
Morphology lives in OPP12OvershootFailDetector.
...
from strategies.paaf.detectors.opp12_overshoot_fail import (
    DEFAULT_ATR_PERIOD,
    DEFAULT_EMA_PERIOD,
    DEFAULT_OVERSHOOT_ATR_MULT,
    DEFAULT_OVERSHOOT_MAX_ATR_MULT,
    DEFAULT_REVERSAL_CLOSE_QUARTER,
    DEFAULT_REVERSAL_MIN_BODY_RATIO,
    DEFAULT_REVERSAL_SHADOW_MIN_RATIO,
    OPP12OvershootFailDetector,
)
...
class StratRevOpp1201Strategy(CtaTemplate):
    """CID_004 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    overshoot_atr_mult = DEFAULT_OVERSHOOT_ATR_MULT
    overshoot_max_atr_mult = DEFAULT_OVERSHOOT_MAX_ATR_MULT
    reversal_shadow_min_ratio = DEFAULT_REVERSAL_SHADOW_MIN_RATIO
    reversal_close_quarter = DEFAULT_REVERSAL_CLOSE_QUARTER
    reversal_min_body_ratio = DEFAULT_REVERSAL_MIN_BODY_RATIO
    risk_reward = 1.0
    max_hold_bars = 50
    fixed_size = 1
    ...
    parameters = [
        ...
        "ema_period",
        "overshoot_atr_mult",
        "overshoot_max_atr_mult",
        "reversal_shadow_min_ratio",
        "reversal_close_quarter",
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP12OvershootFailDetector(
            atr_period=int(self.atr_period),
            ema_period=int(self.ema_period),
            overshoot_atr_mult=float(self.overshoot_atr_mult),
            overshoot_max_atr_mult=float(self.overshoot_max_atr_mult),
            reversal_shadow_min_ratio=float(self.reversal_shadow_min_ratio),
            reversal_close_quarter=float(self.reversal_close_quarter),
            reversal_min_body_ratio=float(self.reversal_min_body_ratio),
        )
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_004; detector=OPP12@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
