"""STRAT_REV_OPP16_01@0.1.0 — PAAF orchestration for OPP16 two-bar reversal.

CID_003 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP16 Detector → levels → Execution → log.
Morphology lives in OPP16TwoBarReversalDetector（unchanged bytes）.
...
class StratRevOpp1601Strategy(CtaTemplate):
    """CID_003 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    risk_reward = 1.0
    max_hold_bars = 50  # 1m bars ≈ 10 × 5m signal bars
    fixed_size = 1
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP16TwoBarReversalDetector(body_ratio=float(self.body_ratio))
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_003; detector=OPP16@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
