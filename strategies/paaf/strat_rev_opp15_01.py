"""STRAT_REV_OPP15_01@0.1.0 — PAAF orchestration for OPP15 wedge Path A.

CID_009 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP15 Detector → levels → Execution → log.
Morphology / FSM live in OPP15WedgePathADetector.
...
class StratRevOpp1501Strategy(CtaTemplate):
    """CID_009 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    wedge_n_min = DEFAULT_WEDGE_N_MIN
    wedge_alpha_threshold = DEFAULT_WEDGE_ALPHA
    wedge_arm_trigger_max_bars = DEFAULT_ARM_MAX_BARS
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP15WedgePathADetector(
            atr_period=int(self.atr_period),
            wedge_n_min=int(self.wedge_n_min),
            wedge_alpha_threshold=float(self.wedge_alpha_threshold),
            wedge_arm_trigger_max_bars=int(self.wedge_arm_trigger_max_bars),
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_009; detector=OPP15@1.0.0; signal=5m; risk=1m)"
        )
        self.load_bar(30)
