"""STRAT_SESS_OPP19_01@0.1.0 — PAAF orchestration for OPP19 OD-Breakout.

CID_007 · Candidate Identity Freeze target.
Strategy only orchestrates: Context → OPP19 Detector → levels → Execution → log.
Morphology + FSM live in OPP19OpeningDriveBreakoutDetector.
...
from strategies.paaf.detectors.opp19_opening_drive_breakout import (
    DEFAULT_ATR_PERIOD,
    DEFAULT_OPENING_DRIVE_BARS,
    DEFAULT_OPENING_DRIVE_MIN_BODY,
    DEFAULT_OPENING_DRIVE_RANGE_ATR_MIN,
    DEFAULT_STRONG_BAR_ATR_MULT,
    DEFAULT_STRONG_BAR_BODY_RATIO,
...
class StratSessOpp1901Strategy(CtaTemplate):
    """CID_007 orchestrator v0.1.0 — no pattern logic inlined."""
    ...
    opening_drive_bars = DEFAULT_OPENING_DRIVE_BARS
    opening_drive_min_body = DEFAULT_OPENING_DRIVE_MIN_BODY
    opening_drive_range_atr_min = DEFAULT_OPENING_DRIVE_RANGE_ATR_MIN
    strong_bar_atr_mult = DEFAULT_STRONG_BAR_ATR_MULT
    strong_bar_body_ratio = DEFAULT_STRONG_BAR_BODY_RATIO
    risk_reward = 1.0
    max_hold_bars = 50
    fixed_size = 1
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.detector = OPP19OpeningDriveBreakoutDetector(
            atr_period=int(self.atr_period),
            opening_drive_bars=int(self.opening_drive_bars),
            opening_drive_min_body=float(self.opening_drive_min_body),
            opening_drive_range_atr_min=float(self.opening_drive_range_atr_min),
            strong_bar_atr_mult=float(self.strong_bar_atr_mult),
            strong_bar_body_ratio=float(self.strong_bar_body_ratio),
        )
    ...
    def on_init(self) -> None:
        self.write_log(
            ...
            f"(CID_007; detector=OPP19@1.0.0 OD-Breakout; signal=5m; risk=1m)"
        )
        self.load_bar(30)
    ...
    def on_rollover_adjust(self, event: RolloverEvent) -> None:
        ...
        self.detector.adjust_or_levels(shift)
