...
> **实现门禁**: 本 RFC **Accepted** 之前，禁止提交 Evaluation 持久化代码；仍禁止 Metric Calculator / ATR / Promotion / Ranking。
...
| ID | 问题 | 默认倾向 |
|----|------|----------|
| RQ1 | Definitions 用 per-id 文件还是聚合 `outcomes.json`/`metrics.json`？ | per-id 文件（append-only 更清晰） |
| RQ2 | KEEP persist gate 放 Repository 还是 Workflow？ | Workflow；Repository 保持无 decision 语义 |
| RQ3 | `EvaluationResult.evidence_id` 与 Evidence 链接时是否必须相等？ | 若两者皆非空，必须相等 |
| RQ4 | save_evaluation 是否校验 outcomes/metrics 的 definition 已 persist？ | 是（引用完整性） |
| RQ5 | 指纹 helpers 是否本切片一并交付？ | 最小：`fingerprint_evaluation_body`；定义指纹可同切片或紧随 |

关闭 RQ1–RQ5 后可将本 RFC 标为 **Accepted**。
...
- [ ] RQ1–RQ5 关闭或显式推迟  
- [ ] 与 Phase 0/1/2 append-only、EQ1–EQ6 无矛盾  
- [ ] Out of Scope 明确排除 Calculator / ATR / Promotion  
- [ ] Commit 边界获同意  
- [ ] `docs/README.md` 已索引  
...
Phase 2.1  Evaluation Persistence           ← 本 RFC
