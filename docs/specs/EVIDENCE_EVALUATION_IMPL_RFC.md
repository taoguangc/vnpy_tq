...
> **Status**: Accepted（Frozen for Phase 2 evaluation contracts）  
...
> **实现门禁**: 仅授权 Evaluation models → repository → workflow link 切片；仍禁止 ATR / Metric Calculator / Sensor Adapter / Promotion。
...
```text
strategies/paaf/
    evidence/              # Phase 0/1 — 不变
    evaluation/
        __init__.py
        models.py          # Phase 2 — NEW
```

| 模块 | 拥有 | 禁止 |
|------|------|------|
| `evaluation/models.py` | Definition/Record/Result 契约、预登记引用校验 | I/O；Sensor；统计库 |
...
@dataclass(frozen=True)
class OutcomeRecord:
    definition_id: str              # 必须引用 OutcomeDefinition.outcome_id
    values: Mapping[str, float | str]
    sample_n: float                 # effective observation count，可为小数
...
- `sample_n` 表示 effective observation count，不要求等于原始行数
...
| 2 | `feat(paaf): add evaluation models` | `evaluation/models.py` 契约 + model tests |
...
| ID | Accepted 决议 |
|----|---------------|
| EQ1 | `evidence_id` 可选；修订链接必须创建新 EvaluationResult，不改旧对象 |
| EQ2 | KEEP 必须引用将要/已经存在的 `evidence_id`，且治理时要求 EvaluationResult 已 persist |
| EQ3 | Phase 2 Evidence 通过显式 metadata `evaluation_id` 链接；多 Evaluation 场景另开 Reference RFC |
| EQ4 | Definitions 属于 experiment 目录；跨实验同 ID 用内容 fingerprint 比对 |
| EQ5 | Phase 2 不内置公式；只记录 `formula_id` |
| EQ6 | `sample_n: float`，表示 effective observation count，可为小数 |
...
- [x] EQ1–EQ6 关闭或显式推迟  
- [x] 与 Phase 0/1 append-only / ID ownership 无矛盾  
- [x] Out of Scope 明确排除 ATR / Promotion / 内置绩效优化  
- [x] Commit 边界获同意  
...
| 2026-07-19 | 1.0.0 | **Accepted**：EQ1–EQ6 关闭；授权 generic evaluation models |
