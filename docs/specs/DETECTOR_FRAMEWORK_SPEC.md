...
## 13. Detector Pipeline Verification（v0.2.4）

验证对象是完整 Pipeline，不是 Demo Alpha：

```text
Context
    ↓
Registry Descriptor → Factory → Detector
    ↓
DetectionResult
    ↓
Opportunity
...
`DetectorPipeline.run(window, context) -> tuple[Opportunity, ...]` 必须：

1. 从 Registry 读取非 Deprecated Descriptor；
2. 按 Capability 检查 Context（market_state / requires）；
3. 通过 Descriptor factory 延迟实例化；
4. 调用 Detector，接受 `DetectionResult | None`；
5. 校验 result 的 detector id / version / status 与 Descriptor 一致；
6. 构造 Opportunity（保留 evidence 与 lineage）；
7. 将 Opportunity 交给 Logger（若配置）。

Pipeline 不负责 Risk / Execution，也不得把 Opportunity 当订单。

### 13.2 `DEMO_MINIMAL`（载体，不是 Alpha）

- 规则：只在**已完成 Bar**满足 `close > open` 时返回 DetectionResult；
- `detector_id/opportunity_id = "DEMO_MINIMAL"`；
- `status = EXPERIMENT`，tag = `DEMO`；
- 必须读取只读 Context 并记录市场状态 / session 审计字段；
- `close <= open` 返回 `None`；
- **禁止** Production、OPP 编号、ADX/ATR/Market State 算法与收益主张。

### 13.3 最小 Logger 消费

`OpportunityLogger` 可用 JSONL 写出 `Opportunity.to_dict()`，只验证消费与审计路径。
它不替代后续 Evidence Logger RFC，也不更改交易结果。

### 13.4 Registry API 冻结

v0.2.3 核心 API 不增加普通 Helper。查询统一走 `find(...)` Capability Query；新需求优先扩展查询参数语义，须先修订 Spec。
...
| **v0.2.4** | Detector Pipeline Verification + `DEMO_MINIMAL` + Opportunity Logger |
...
- Pipeline 七步：注册、实例化、Context、DetectionResult、Opportunity、Contract、Logger
- `DEMO_MINIMAL` 仅 `close > open`，close<=open 返回 None；不得进入 Production
...
| 2026-07-19 | 1.3.0 | v0.2.4 改为 Detector Pipeline Verification；冻结七步验收与 Minimal Demo |
