...
> **实现门禁**: 本 RFC **Accepted** 之前，禁止提交 Sensor 框架代码；仍禁止 ATRCompression / TrendDetector / Feature ranking / Trading signal。
...
| ID | 问题 | 默认倾向 |
|----|------|----------|
| SQ1 | `SensorStatus` 独立 Enum 还是复用 `DetectorStatus`？ | 独立 `SensorStatus`（同名值，避免语义耦合） |
| SQ2 | `observe` 的 `window` 类型用 Protocol 还是 `Mapping`？ | Phase 3.0：`Mapping[str, Any]` + fixture；Adapter RFC 再接 vn.py |
| SQ3 | DEMO sensor id 命名？ | `demo_constant` / version `1.0` |
| SQ4 | Feature 行写入放 `sensors/artifact.py` 还是扩 Repository？ | `sensors/artifact.py`；Repository 只存 metadata/refs |
| SQ5 | Always emit 冷启动：缺失值用跳过还是 diagnostics？ | DEMO：always emit；warmup 用 diagnostics `warmup_state` |
| SQ6 | Phase 3.0 是否强制接入 Evaluation？ | 否；只要求能接到 ArtifactReference；Evaluation 联调可另测 |

关闭 SQ1–SQ6 后可将本 RFC 标为 **Accepted**。
...
- [ ] SQ1–SQ6 关闭或显式推迟  
- [ ] 与 `FEATURE_SENSOR_SPEC.md` Q1–Q5 / Decision 015 无矛盾  
- [ ] Out of Scope 明确排除 ATR / ranking / trading  
- [ ] Commit 边界获同意  
- [ ] `docs/README.md` 已索引  
