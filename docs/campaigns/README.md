# Campaigns pointer — LEAVE（L2）· CID_003–014 PAUSED

```text
Posture: LEAVE_SHELF_EXHAUSTED（V0_1 + L1 + L2 · bare ≠ reopen）
CID_003–014: PAUSED · Alpha NONE
CID_015: NOT opened
Delegation-25BO: STOP
```

## Pointers

| Item | Status |
|------|--------|
| Leave L2 | [REAFFIRM](../research/STRATEGY_POSTURE_LEAVE_REAFFIRMATION_25BO.md) |
| Leave L1 | [REAFFIRM](../research/STRATEGY_POSTURE_LEAVE_REAFFIRMATION_25BN.md) |
| Leave posture | [ACTIVE](../research/STRATEGY_POSTURE_LEAVE_SHELF_EXHAUSTED.md) |
| CID_014 Pause | [CPD](../research/STRATEGY_CAMPAIGN_PAUSE_CID_014.md) |

## Reopen（须明确唤醒句）

```text
Authorize NSAD Setup C (explicit multi-axis)
Authorize scoped extract from pa_minimal
Resume CID_0XX (scoped)
```
