...
- **日期**：2026-07-20
- **状态**：Accepted
- **主题**：Stable Contracts, Replaceable Infrastructure（契约稳定，基础设施可替换）
- **背景**：ABR-001 已建立 Architecture Baseline v1。Evidence Domain 与治理已冻结。下一风险是把
  JSON/文件实现误当成合同，或为 Dashboard 反向改 Domain。需要把「可替换的是实现、不可变的是契约」
  升格为基础设施总原则，覆盖 Storage 以及未来 Validation / Query / Execution Backend。
- **决策**：采纳下列 Architecture Principles（写入 `docs/02_ARCHITECTURE.md`），并 Accepted：
  - `docs/specs/APPEND_ONLY_STORAGE_SPEC.md` v1.0.0
  - `docs/specs/PROJECTION_LAYER_SPEC.md` v1.0.0

### Principle 3 — Storage is Replaceable; Contracts are Stable.

Repository 可从 JSON 文件、DuckDB、SQLite、PostgreSQL、Object Storage 等替换，只要遵守
Append-only Storage Contract（及 Evidence Domain）。换存储不得要求改 Domain / 改本 ADR
（除非新 ADR 覆盖）。验收见 Storage Spec § Replaceability Checklist。

现有 `save_*` 若为 create-only 语义，即合规；**不强制**本轮 rename（REPO-001 Backlog；
倾向未来领域名 `record`）。

### Principle 4 — Projection is Derived; Domain is Authoritative.

Portfolio / Timeline / Dashboard / Analytics / Knowledge Graph 等均为 **Projection**：
由 Domain 派生、只读。禁止 Projection 修改 Repository / Domain。不得为面板「好看」改权威记录。

### 文档状态词（DOC-001）

规范与 RFC 的 **Status** 字段只允许：

```text
Draft | Accepted | Deprecated | Archived
```

禁止用「接口草案」等临时措辞充当 Status，以免 Agent 误判。

- **原因**：冻结基础设施契约层，使后续能力建设不必反复回头改地基。  
- **后果**：v0.3 进入「Contract Freeze」节点；下一工程为 Repository Refinement（对齐 Contract）
  与只读 Portfolio Projection；完成后 ABR-002。  
