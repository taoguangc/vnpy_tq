"""CTX_CID003_EXP005 — H_CTX_RISK_COMP temporal OOS（rb/2025 · B0 vs B1）.

Identical RISK @0.2.0 + Filter F1 as EXP004; period is the OOS dimension.
...
EXPERIMENT_ID = "CTX_CID003_EXP005"
...
class Opp16CtxFilterV020Exp005(Opp16CtxFilterV020):
...
def _run(strategy_cls) -> tuple[list[dict], list[dict], dict]:
    ...
    if not bars:
        raise RuntimeError("no bars for CTX_CID003_EXP005")
...
def _evaluate(n0: int, n1: int, d: int, source_ok: bool, param_ok: bool) -> tuple[str, str]:
    ...
    if n1 == n0 and d == 0:
        return "HOLD", "filter inert under RISK OOS（N1==N0 and D==0）"
    if n1 != n0 or d >= 1:
        return (
            "KEEP",
            f"filter active under RISK OOS：N0={n0} N1={n1} D={d}; "
...
def main() -> int:
    ...
    print("running B0 baseline（RISK V020 unfiltered · 2025）...")
    rows0, _, meta0 = _run(StratRevOpp1601StrategyV020)
    print(f"B0 N0={len(rows0)}")

    print("running B1 filtered（RISK V020+F1 · 2025）...")
    rows1, denials, meta1 = _run(Opp16CtxFilterV020Exp005)
    ...
    run_meta = {
        ...
        "authorization": "Delegation-25K（Authorize CTX_CID003_EXP005 H_CTX_RISK_COMP OOS）",
