"""STRAT_BS02_EXP005 — H_MECH smoke for v0.1.1 rollover repair."""
...
EXPERIMENT_ID = "STRAT_BS02_EXP005"
...
def main() -> int:
    ...
    if not source_ok or missing_hook_warn:
        outcome, reason = "REVERT", "hash mismatch or missing-hook WARN present"
    elif not rows or attributed < 1:
        outcome, reason = "HOLD", "no auditable exits with allowed reasons"
    else:
        outcome, reason = "KEEP", (
            f"auditable_trades={len(rows)}; missing_hook_warn={missing_hook_warn}; "
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        w = csv.DictWriter(fh, fieldnames=list(rows[0].keys()) if rows else ["experiment_id"])
        w.writeheader()
        for row in rows:
            out = dict(row)
    ...
    print(f"missing_hook_warn={missing_hook_warn} adjust_log_seen={adjust_seen}")
    return 0
