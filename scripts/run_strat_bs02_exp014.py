"""STRAT_BS02_EXP014 — H_EDGE OOS for @0.1.1（rb/2025）.

Temporal completeness vs EXP013 REVERT — not a rescue attempt.
Gates identical to EXP013. Do NOT run without Observation auth.

≠ Alpha Candidate · ≠ parameter change · ≠ metric shopping.
"""
...
EXPERIMENT_ID = "STRAT_BS02_EXP014"
...
def _one_sided_ttest_gt_zero(values: list[float]) -> tuple[float, float]:
    """Return (t_stat, approximate one-sided p for H1: mean > 0)."""
    n = len(values)
    if n < 2:
    ...
    # Normal approximation for large n（min_n=50）
    z = t_stat
...
def main() -> int:
    ...
    meta = {
        ...
        "authorization": "Authorize Offline Alpha Evidence Observation（required at run）",
        "hypothesis_family": "H_EDGE_OOS",
        "not_hypothesis": ["H_MECH", "H_CAPITAL_GATE", "H_ALPHA_VERIFIED", "EXP013_RESCUE"],
        "oos_vs": "STRAT_BS02_EXP013/rb/2024",
        "prior_exp013_outcome": "REVERT",
        "outcome": outcome,
