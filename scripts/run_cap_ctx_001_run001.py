"""CAP_CTX_001_RUN001 — Controlled Observation + Evaluation (Appendix A).

Produces evaluation artifacts only. Does NOT claim Knowledge / Gate / Alpha.
"""
...
# Causal warmup before Full (W=20, L=240)
...
def _update_manifest_start(revision: str) -> None:
    ...
    manifest["observation_execution"] = {
        "authorized_command": "Authorize Observation Execution for CAP_CTX_001_RUN001",
        "started_at_utc": now,
        "code_revision_at_start": revision,
    }
    manifest["environment_identity"]["code_revision_at_observation_start"] = revision
    manifest["note"] = (
        "Observation execution authorized and started under Appendix A. "
        "No Knowledge / Gate / Alpha claim from this Manifest."
    )
...
def _check_coverage_gap(df: pd.DataFrame, symbol: str) -> dict:
    ...
    return {
        ...
        "gap_policy": (
            "STOP iff any calendar month in 2024-01..2025-12 has zero bars; "
...
def main() -> int:
    ...
    print(f"[CAP_CTX_001_RUN001] start revision={revision}", flush=True)
    ...
    if primary_e1.status == "HOLD" or e2.get("status") == "HOLD":
        ...
        knowledge_candidate = "K004_CANDIDATE"
        decision = "HOLD"
    elif not e1_pass_primary:
        knowledge_candidate = "K002_CANDIDATE"
        decision = "REVERT"
    else:
        knowledge_candidate = "K002_OR_K003_CANDIDATE"
        decision = "HOLD"
    ...
    evaluation = {
        ...
        "claim_boundary": {
            "allowed": (
                "Evidence supports / does not support Context Capability "
                "hypothesis under registered conditions (Candidate only)."
            ),
            "forbidden": [
                ...
                "RC001 Accepted",
                "Accepted Knowledge without Review",
            ],
        },
    ...
    print(
        f"[done] decision={decision} knowledge_candidate={knowledge_candidate}",
        flush=True,
    )
