"""CTX_CID002_EXP001 — H_CTX_FILTER Observation（B0 vs B1）.

B0: MECH @0.1.1 unfiltered
B1: MECH @0.1.1 + A1 Context Filter F1（expansion-only）
...
EXPERIMENT_ID = "CTX_CID002_EXP001"
...
def _run(strategy_cls) -> tuple[list[dict], list[dict], dict]:
    ...
    # Stamp denial datetime from last known if missing — optional
    meta = {
...
def _evaluate(n0: int, n1: int, d: int, source_ok: bool, param_ok: bool) -> tuple[str, str]:
    ...
    if n1 != n0 or d >= 1:
        return (
            "KEEP",
            f"filter active：N0={n0} N1={n1} D={d}; "
...
def main() -> int:
    ...
    print("running B0 baseline（unfiltered V011）...")
    rows0, _, meta0 = _run(BrooksScalpPaafStrategyV011)
    print(f"B0 N0={len(rows0)}")

    print("running B1 filtered（V011+F1）...")
    ...
    # Sample context tag distribution from denials + note
